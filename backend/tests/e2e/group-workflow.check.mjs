...
import ExpensesRepository from '../../src/modules/itinerary/itinerary-expenses.repository.js';
import SettlementsRepository from '../../src/modules/itinerary/itinerary-expense-settlements.repository.js';
...
try {
  ...
  await call('post',`${base}/expense-settlements`,0,{toUserId:users[1],amount:50,currencyCode:'INR'},201);
  const before=await call('get',`${base}/expense-balances`,0);
  await call('delete',`/groups/${saved.group.id}/itinerary`,0,undefined,409);
  ...
  await call('delete',`/groups/${saved.group.id}/members/${users[1]}`,0);
  assert.equal((await call('delete',`${base}/group/members/${users[1]}`,0)).updated,false);
  await call('get',base,1,undefined,404);
  ...
  await call('patch',`${base}/status`,0,{status:'PLANNED'},409);
  const unlinked=(await call('post','/groups',0,{name:'Invite before itinerary'})).group;
  const invitePath=`/groups/${unlinked.id}/invitations`;
  await call('post',invitePath,null,{userId:users[1]},401);
  await call('post','/groups/bad/invitations',0,{userId:users[1]},400);
  await call('post',invitePath,1,{userId:users[0]},404);
  await call('post',invitePath,0,{userId:users[0]},400);
  await call('post',invitePath,0,{userId:users[2]},403);
  const standaloneInvite=await call('post',invitePath,0,{userId:users[1]});
  assert.equal(standaloneInvite.invitation.itineraryId,null);
  const repeatedInvite=await call('post',invitePath,0,{userId:users[1]});
  assert.equal(repeatedInvite.created,false);
  assert.equal(repeatedInvite.invitation.id,standaloneInvite.invitation.id);
  assert((await call('get','/users/me/group-invitations?status=PENDING',1)).invitations.some(i=>i.id===standaloneInvite.invitation.id && i.itineraryId===null));
  await call('get',`/groups/${unlinked.id}`,1,undefined,404);
  const responsePath=`/users/me/group-invitations/${standaloneInvite.invitation.id}`;
  await call('patch',responsePath,2,{status:'ACCEPTED'},404);
  const acceptedStandalone=await call('patch',responsePath,1,{status:'ACCEPTED'});
  assert.equal(acceptedStandalone.invitation.itineraryId,null);
  assert.equal((await call('patch',responsePath,1,{status:'ACCEPTED'})).updated,false);
  assert.equal((await call('get',`/groups/${unlinked.id}`,1)).totalCount,2);
  await call('post',invitePath,0,{userId:users[1]},409);
  const removalPath=`/groups/${unlinked.id}/members/${users[1]}`;
  await call('delete',removalPath,null,undefined,401);
  await call('delete','/groups/bad/members/bad',0,undefined,400);
  await call('delete',`/groups/${unlinked.id}/members/bad`,0,undefined,400);
  await call('delete',removalPath,1,undefined,404);
  await call('delete',removalPath,2,undefined,404);
  await call('delete',`/groups/${randomUUID()}/members/${users[1]}`,0,undefined,404);
  await call('delete',`/groups/${unlinked.id}/members/${users[2]}`,0,undefined,404);
  await call('delete',`/groups/${unlinked.id}/members/${users[0].toUpperCase()}`,0,undefined,409);
  const removedStandalone=await call('delete',removalPath,0);
  assert.equal(removedStandalone.updated,true);
  assert.equal(removedStandalone.itineraryId,null);
  assert.equal(removedStandalone.status,'REMOVED');
  assert.equal((await call('delete',removalPath,0)).updated,false);
  await call('get',`/groups/${unlinked.id}`,1,undefined,404);
  assert.equal((await call('get',`/groups/${unlinked.id}`,0)).totalCount,1);
  const reinvite=await call('post',invitePath,0,{userId:users[1]});
  await call('patch',`/users/me/group-invitations/${reinvite.invitation.id}`,1,{status:'ACCEPTED'});
  const later=(await call('post','/itineraries',0,payload,201)).itinerary;
  const leavePath=`/groups/${unlinked.id}/leave`;
  await call('post',leavePath,null,{},401);
  await call('post','/groups/bad/leave',1,{},400);
  await call('post',leavePath,1,{userId:users[0]},400);
  await call('post',leavePath,2,{},404);
  await call('post',`/groups/${randomUUID()}/leave`,1,{},404);
  await call('post',leavePath,0,{},409);
  const leftStandalone=await call('post',leavePath,1,{});
  assert.equal(leftStandalone.itineraryId,null);
  assert.equal(leftStandalone.userId,users[1]);
  assert.equal(leftStandalone.updated,true);
  assert.equal((await call('post',leavePath,1,{})).updated,false);
  await call('get',`/groups/${unlinked.id}`,1,undefined,404);
  assert.equal((await call('get',`/groups/${unlinked.id}`,0)).totalCount,1);
  const returnInvite=await call('post',invitePath,0,{userId:users[1]});
  await call('patch',`/users/me/group-invitations/${returnInvite.invitation.id}`,1,{status:'ACCEPTED'});
  await call('patch',`/itineraries/${later.id}/status`,0,{status:'UPCOMING'});
  ...
  const linkedBalances=await call('get',`/itineraries/${later.id}/expense-balances`,0);
  assert.equal((await call('delete',removalPath,0)).itineraryId,later.id);
  await call('get',`/itineraries/${later.id}/expenses/dashboard`,1,undefined,404);
  assert.equal((await call('get',`/itineraries/${later.id}/expense-participants`,0)).totalCount,1);
  assert.deepEqual(await call('get',`/itineraries/${later.id}/expense-balances`,0),linkedBalances);
  const linkedReinvite=await call('post',invitePath,0,{userId:users[1]});
  await call('patch',`/users/me/group-invitations/${linkedReinvite.invitation.id}`,1,{status:'ACCEPTED'});
  assert.equal((await call('post',leavePath,1,{})).itineraryId,later.id);
  assert.equal((await call('post',leavePath,1,{})).updated,false);
  await call('get',`/itineraries/${later.id}/expenses/dashboard`,1,undefined,404);
  assert.equal((await call('get',`/itineraries/${later.id}/expense-participants`,0)).totalCount,1);
  assert.deepEqual(await call('get',`/itineraries/${later.id}/expense-balances`,0),linkedBalances);
  const unlinkPath=`/groups/${unlinked.id}/itinerary`;
  await call('delete',unlinkPath,null,undefined,401);
  await call('delete','/groups/bad/itinerary',0,undefined,400);
  await call('delete',unlinkPath,1,undefined,404);
  await call('delete',`/groups/${randomUUID()}/itinerary`,0,undefined,404);
  const again=await call('post',invitePath,0,{userId:users[1]});
  await call('patch',`/users/me/group-invitations/${again.invitation.id}`,1,{status:'ACCEPTED'});
  const detached=await call('delete',unlinkPath,0);
  assert.equal(detached.itineraryId,later.id);
  assert.equal(detached.group.itineraryId,null);
  assert.equal(detached.updated,true);
  const {rows:[detachedTrip]}=await client.query('SELECT id FROM trip.trips WHERE itinerary_id=$1',[later.id]);
  await assert.rejects(ExpensesRepository.create({tripId:detachedTrip.id,createdBy:users[1],
    input:{paidBy:users[1]},splits:[]}),error=>error.statusCode===422);
  await assert.rejects(SettlementsRepository.create({tripId:detachedTrip.id,userId:users[1],
    input:{toUserId:users[0]}}),error=>error.statusCode===422);
  assert.equal((await call('get',`/groups/${unlinked.id}`,1)).totalCount,2);
  await call('get',`/itineraries/${later.id}/expenses/dashboard`,1,undefined,404);
  assert.equal((await call('get',`/itineraries/${later.id}/expense-participants`,0)).totalCount,1);
