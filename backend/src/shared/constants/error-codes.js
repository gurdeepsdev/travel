const ErrorCodes = Object.freeze({

    AUTH: Object.freeze({

        INVALID_OTP: "AUTH.INVALID_OTP",

        OTP_EXPIRED: "AUTH.OTP_EXPIRED",

        INVALID_REFRESH_TOKEN: "AUTH.INVALID_REFRESH_TOKEN",

        SESSION_EXPIRED: "AUTH.SESSION_EXPIRED",

        IDENTITY_NOT_FOUND: "AUTH.IDENTITY_NOT_FOUND",

        IDENTITY_NOT_VERIFIED: "AUTH.IDENTITY_NOT_VERIFIED",

        INVALID_ACCESS_TOKEN: "AUTH.INVALID_ACCESS_TOKEN",


        

    }),

    USER: Object.freeze({

        NOT_FOUND: "USER.NOT_FOUND",

        USERNAME_ALREADY_EXISTS: "USER.USERNAME_ALREADY_EXISTS",

        PHONE_ALREADY_EXISTS: "USER.PHONE_ALREADY_EXISTS",

        EMAIL_ALREADY_EXISTS: "USER.EMAIL_ALREADY_EXISTS",

        ACCOUNT_SUSPENDED: "USER.ACCOUNT_SUSPENDED",

        ACCOUNT_DEACTIVATED: "USER.ACCOUNT_DEACTIVATED"

    }),

 PROFILE: {
  PHOTO_NOT_ALLOWED:
    "PROFILE.PHOTO_NOT_ALLOWED",

  PHOTO_INVALID_TYPE:
    "PROFILE.PHOTO_INVALID_TYPE",

  PHOTO_TOO_LARGE:
    "PROFILE.PHOTO_TOO_LARGE",

  PHOTO_UPLOAD_FAILED:
    "PROFILE.PHOTO_UPLOAD_FAILED",

  COUNTRY_NOT_FOUND:
    "PROFILE.COUNTRY_NOT_FOUND",

  CITY_NOT_FOUND:
    "PROFILE.CITY_NOT_FOUND",

  CITY_COUNTRY_MISMATCH:
    "PROFILE.CITY_COUNTRY_MISMATCH",
},

CONNECTION: Object.freeze({
  TARGET_NOT_AVAILABLE:
    "CONNECTION.TARGET_NOT_AVAILABLE",

  SELF_REQUEST_NOT_ALLOWED:
    "CONNECTION.SELF_REQUEST_NOT_ALLOWED",

  ALREADY_CONNECTED:
    "CONNECTION.ALREADY_CONNECTED",

  REQUEST_ALREADY_RECEIVED:
    "CONNECTION.REQUEST_ALREADY_RECEIVED",

  REQUEST_NOT_FOUND:
    "CONNECTION.REQUEST_NOT_FOUND",

  CONNECTION_NOT_FOUND:
    "CONNECTION.NOT_FOUND",
}),


BLOCK: Object.freeze({
  TARGET_NOT_AVAILABLE:
    "BLOCK.TARGET_NOT_AVAILABLE",

  SELF_BLOCK_NOT_ALLOWED:
    "BLOCK.SELF_BLOCK_NOT_ALLOWED",
}),

CITY: Object.freeze({
  NOT_FOUND:
    "CITY.NOT_FOUND",
}),

MEMORY: Object.freeze({
  ASSET_NOT_ALLOWED:
    "MEMORY.ASSET_NOT_ALLOWED",

  MEDIA_REQUIRED:
    "MEMORY.MEDIA_REQUIRED",

  MEDIA_INVALID_TYPE:
    "MEMORY.MEDIA_INVALID_TYPE",

  MEDIA_TOO_LARGE:
    "MEMORY.MEDIA_TOO_LARGE",

  MEDIA_UPLOAD_FAILED:
    "MEMORY.MEDIA_UPLOAD_FAILED",

  NOT_FOUND:
    "MEMORY.NOT_FOUND",
}),

REPORT: Object.freeze({
  TARGET_NOT_AVAILABLE:
    "REPORT.TARGET_NOT_AVAILABLE",

  SELF_REPORT_NOT_ALLOWED:
    "REPORT.SELF_REPORT_NOT_ALLOWED",
}),


VISITED_PLACE: Object.freeze({
  EVIDENCE_REQUIRED:
    "VISITED_PLACE.EVIDENCE_REQUIRED",

  EVIDENCE_INVALID_TYPE:
    "VISITED_PLACE.EVIDENCE_INVALID_TYPE",

  EVIDENCE_TOO_LARGE:
    "VISITED_PLACE.EVIDENCE_TOO_LARGE",

  EVIDENCE_UPLOAD_FAILED:
    "VISITED_PLACE.EVIDENCE_UPLOAD_FAILED",

  PLACE_NOT_AVAILABLE:
    "VISITED_PLACE.PLACE_NOT_AVAILABLE",

  DUPLICATE_EVIDENCE:
    "VISITED_PLACE.DUPLICATE_EVIDENCE",

  EVIDENCE_REJECTED:
    "VISITED_PLACE.EVIDENCE_REJECTED",

  COLLECTION_NOT_FOUND:
    "VISITED_PLACE.COLLECTION_NOT_FOUND",

  VERIFICATION_NOT_FOUND:
    "VISITED_PLACE.VERIFICATION_NOT_FOUND",

  COLLECTION_NOT_VERIFIED:
    "VISITED_PLACE.COLLECTION_NOT_VERIFIED",

  PREFERENCE_LIMIT_REACHED:
    "VISITED_PLACE.PREFERENCE_LIMIT_REACHED",
}),

MEDIA: Object.freeze({
  ASSET_NOT_FOUND:
    "MEDIA.ASSET_NOT_FOUND",

  CONTENT_UNAVAILABLE:
    "MEDIA.CONTENT_UNAVAILABLE",
}),


    PAGE: Object.freeze({

        NOT_FOUND: "PAGE.NOT_FOUND",

        ACCESS_DENIED: "PAGE.ACCESS_DENIED",

        SLUG_ALREADY_EXISTS: "PAGE.SLUG_ALREADY_EXISTS"

    }),

    ITINERARY: Object.freeze({

        NOT_FOUND: "ITINERARY.NOT_FOUND",

        INVALID_STATUS_TRANSITION:
  "ITINERARY.INVALID_STATUS_TRANSITION",

        CHANGE_REQUEST_NOT_FOUND:
  "ITINERARY.CHANGE_REQUEST_NOT_FOUND",

        CHANGE_REQUEST_STALE:
  "ITINERARY.CHANGE_REQUEST_STALE",

        TRIP_NOT_STARTED:
  "ITINERARY.TRIP_NOT_STARTED",

        VAULT_FILE_REQUIRED:
  "ITINERARY.VAULT_FILE_REQUIRED",

        VAULT_INVALID_FILE_TYPE:
  "ITINERARY.VAULT_INVALID_FILE_TYPE",

        VAULT_FILE_TOO_LARGE:
  "ITINERARY.VAULT_FILE_TOO_LARGE",

        VAULT_GROUP_REQUIRED:
  "ITINERARY.VAULT_GROUP_REQUIRED",

        EXPENSE_PARTICIPANT_NOT_ELIGIBLE:
  "ITINERARY.EXPENSE_PARTICIPANT_NOT_ELIGIBLE",

        EXPENSE_PARTICIPANT_ALREADY_EXISTS:
  "ITINERARY.EXPENSE_PARTICIPANT_ALREADY_EXISTS",

        EXPENSE_PARTICIPANT_NOT_FOUND:
  "ITINERARY.EXPENSE_PARTICIPANT_NOT_FOUND",

        EXPENSE_OWNER_REMOVAL_FORBIDDEN:
  "ITINERARY.EXPENSE_OWNER_REMOVAL_FORBIDDEN",

        EXPENSE_SPLIT_INVALID:
  "ITINERARY.EXPENSE_SPLIT_INVALID",

        EXPENSE_PARTICIPANT_INVALID:
  "ITINERARY.EXPENSE_PARTICIPANT_INVALID",

        EXPENSE_RECEIPT_INVALID:
  "ITINERARY.EXPENSE_RECEIPT_INVALID",

        EXPENSE_NOT_FOUND:
  "ITINERARY.EXPENSE_NOT_FOUND",

        EXPENSE_SETTLEMENT_NOT_FOUND:
  "ITINERARY.EXPENSE_SETTLEMENT_NOT_FOUND",

        EXPENSE_SETTLEMENT_INVALID:
  "ITINERARY.EXPENSE_SETTLEMENT_INVALID",

        EXPENSE_REMINDER_INVALID:
  "ITINERARY.EXPENSE_REMINDER_INVALID",

        EXPENSE_REMINDER_RATE_LIMITED:
  "ITINERARY.EXPENSE_REMINDER_RATE_LIMITED"

    }),

    GROUP: Object.freeze({
        MEMBER_NOT_FOUND: "GROUP.MEMBER_NOT_FOUND",
        OWNER_REMOVAL_FORBIDDEN: "GROUP.OWNER_REMOVAL_FORBIDDEN",
        INVITATION_NOT_FOUND: "GROUP.INVITATION_NOT_FOUND",
        INVITATION_RESOLVED: "GROUP.INVITATION_RESOLVED",
        INVITATION_EXPIRED: "GROUP.INVITATION_EXPIRED",
        SELF_INVITATION: "GROUP.SELF_INVITATION",
        CONNECTION_REQUIRED: "GROUP.CONNECTION_REQUIRED",
        ALREADY_MEMBER: "GROUP.ALREADY_MEMBER",
        IMAGE_INVALID_TYPE: "GROUP.IMAGE_INVALID_TYPE",
        IMAGE_TOO_LARGE: "GROUP.IMAGE_TOO_LARGE",
        IMAGE_UPLOAD_FAILED: "GROUP.IMAGE_UPLOAD_FAILED",

        NOT_FOUND:
  "GROUP.NOT_FOUND",

        ITINERARY_ALREADY_LINKED:
  "GROUP.ITINERARY_ALREADY_LINKED",

    }),

    COMMON: Object.freeze({

        VALIDATION_FAILED: "COMMON.VALIDATION_FAILED",

        INTERNAL_SERVER_ERROR: "COMMON.INTERNAL_SERVER_ERROR",

        INVALID_CURSOR:
  "COMMON.INVALID_CURSOR",

    }),
    
 
      
      COMMENT: Object.freeze({
          NOT_FOUND:
    "COMMENT.NOT_FOUND",
        PARENT_NOT_FOUND:
          "COMMENT.PARENT_NOT_FOUND",
        CREATE_FAILED:
          "COMMENT.CREATE_FAILED",
      }),

      POST: Object.freeze({
      NOT_FOUND:
        "POST.NOT_FOUND",

      REACTION_NOT_ALLOWED:
        "POST.REACTION_NOT_ALLOWED",

      REPOST_NOT_ALLOWED:
        "POST.REPOST_NOT_ALLOWED",

      CREATE_FAILED:
        "POST.CREATE_FAILED",

      PLACE_NOT_ALLOWED:
        "POST.PLACE_NOT_ALLOWED",

      ASSET_NOT_ALLOWED:
        "POST.ASSET_NOT_ALLOWED",

      ITINERARY_NOT_ALLOWED:
        "POST.ITINERARY_NOT_ALLOWED",

      TAGGED_USER_NOT_ALLOWED:
        "POST.TAGGED_USER_NOT_ALLOWED",

      MEDIA_INVALID_TYPE:
        "POST.MEDIA_INVALID_TYPE",

      MEDIA_TOO_LARGE:
        "POST.MEDIA_TOO_LARGE",

      MEDIA_UPLOAD_FAILED:
        "POST.MEDIA_UPLOAD_FAILED",
    }),


});

export default ErrorCodes;
