...
export async function lockExpenseTrip(client, tripId) {
  ...
  return trip.itinerary_id;
}

export async function lockExpenseRecord(client, expenseId) {
  const { rows: [expense] } = await client.query(
    'SELECT trip_id FROM trip.trip_expenses WHERE id=$1::uuid', [expenseId]);
  if (expense) {
    await lockExpenseTrip(client, expense.trip_id);
  }
}

export async function lockExpenseParticipants(client, tripId, userIds) {
  const itineraryId = await lockExpenseTrip(client, tripId);
